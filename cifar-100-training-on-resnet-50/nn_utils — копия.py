...
def train(train_dataloader, model, loss_fn, optimizer, logging_frequency, device):
    ...
    for batch, (X,y) in enumerate(train_dataloader):
        ...
        preds = yhat.argmax(dim=1)  # class with highest logit / prob
    ...
    training_accuracy = correct / num_samples
...
def test(epoch, test_dataloader, model, loss_fn, device):
    ...
    testing_accuracy = correct / num_samples
    print(f'End of epoch {epoch+1}\n Accuracy: {(100*testing_accuracy):.2f}%, Average loss: {test_loss:.4f}\n')
